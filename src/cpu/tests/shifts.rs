...
fn test_asl() {
    // Values/flags
    let (mut cpu, mut mem) = test_cpu(&vec![ASL_ACC; 10]);
    ...
    mem[abs as usize] = 1 << 2;
    mem[(abs + x as u16) as usize] = 1 << 3;
    ...
    assert_eq!(1 << 1, mem[zpg as usize]);
    assert_eq!(1 << 2, mem[(zpg + x) as usize]);
    assert_eq!(1 << 3, mem[abs as usize]);
    assert_eq!(1 << 4, mem[(abs + x as u16) as usize]);
}

#[test]
fn test_lsr() {
    // Values/flags
    let (mut cpu, mut mem) = test_cpu(&vec![LSR_ACC; 10]);
    cpu.reg_a = 0b1111_1111;
    assert_eq!(2, cpu.run_one(&mut mem));
    ...
    mem[abs as usize] = 1 << 3;
    mem[(abs + x as u16) as usize] = 1 << 4;
    ...
    assert_eq!(1 << 0, mem[zpg as usize]);
    assert_eq!(1 << 1, mem[(zpg + x) as usize]);
    assert_eq!(1 << 2, mem[abs as usize]);
    assert_eq!(1 << 3, mem[(abs + x as u16) as usize]);
}

#[test]
fn test_rol() {
    // Values/flags
    let (mut cpu, mut mem) = test_cpu(&vec![ROL_ACC; 10]);
...
fn test_ror() {
    // Values/flags
    let (mut cpu, mut mem) = test_cpu(&vec![ROR_ACC; 10]);
    cpu.reg_a = 0b0000_0001;
    assert_eq!(2, cpu.run_one(&mut mem));
    assert_eq!(0b0000_0000, cpu.reg_a);
    assert_zn!(cpu, true, false);
