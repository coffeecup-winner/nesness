...
fn test_inc() {
    // Values/flags
    for i in 0..=0xff {
        let (mut cpu, mut mem) = test_cpu(&vec![INC_ZPG, 0x00]);
        mem[0x00] = i;
        let expected = i.wrapping_add(1);
        cpu.run_one(&mut mem);
        assert_eq!(expected, mem[0x00]);
    ...
    assert_eq!(v + 1, mem[zpg as usize]);
    assert_eq!(v + 2, mem[(zpg + x) as usize]);
    assert_eq!(v + 3, mem[abs as usize]);
    assert_eq!(v + 4, mem[(abs + x as u16) as usize]);
}
...
fn test_inx() {
    // Values/flags
    for i in 0..=0xff {
        let (mut cpu, mut mem) = test_cpu(&vec![INX_IMP]);
        cpu.reg_x = i;
        let expected = i.wrapping_add(1);
...
fn test_iny() {
    // Values/flags
    for i in 0..=0xff {
        let (mut cpu, mut mem) = test_cpu(&vec![INY_IMP]);
        cpu.reg_y = i;
        let expected = i.wrapping_add(1);
        assert_eq!(2, cpu.run_one(&mut mem));
...
fn test_dec() {
    // Values/flags
    for i in 0..=0xff {
        let (mut cpu, mut mem) = test_cpu(&vec![DEC_ZPG, 0x00]);
        mem[0x00] = i;
...
fn test_dex() {
    // Values/flags
    for i in 0..=0xff {
        let (mut cpu, mut mem) = test_cpu(&vec![DEX_IMP]);
        cpu.reg_x = i;
        let expected = i.wrapping_sub(1);
        assert_eq!(2, cpu.run_one(&mut mem));
        assert_eq!(expected, cpu.reg_x);
...
fn test_dey() {
    // Values/flags
    for i in 0..=0xff {
        let (mut cpu, mut mem) = test_cpu(&vec![DEY_IMP]);
        cpu.reg_y = i;
        let expected = i.wrapping_sub(1);
        assert_eq!(2, cpu.run_one(&mut mem));
        assert_eq!(expected, cpu.reg_y);
